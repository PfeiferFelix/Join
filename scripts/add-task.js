...
function handleFormSubmit() {                                           // Add event listeners to the submit and clear buttons to handle form submission and clearing of the form, respectively
...
function handleSubmit(form) {   // Handle form submission by validating the form, showing a toast message if valid, resetting the form, and redirecting to the boards page after a short delay
    clearErrors();
    if (validateForm()) {
        showToast();
        form.reset();
        setTimeout(() => { window.location.href = 'boards.html'; }, 2000);  // Redirect to the boards page after a short delay to allow the user to see the toast message before navigating away from the form
    }
}
...
function validateForm() {
    ...
    requiredFields.forEach(field => {
        if (!field.value.trim()) {      // Check if the field is empty or contains only whitespace. If it does, show an error message and set isValid to false to indicate that the form is not valid.
            showError(field);
...
function clearErrors() {
    ...
    document.querySelectorAll('[required]').forEach(field => {                          // Reset the border color of all required fields to the default color to remove any red borders that were applied to indicate errors, providing a visual reset for the form after clearing or successfully submitting it.
